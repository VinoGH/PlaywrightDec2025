...
test("Edit Individual", async({page})=>
{
  ...
  // await page.locator(".app-dnd-identifier one-app-launcher-app-tile").last().waitFor()
   ...
   await page.locator("[name='Individual-search-input']").fill("vinuramsay")
   ...
   await page.locator("[placeholder='First Name']").fill("vinura")
   //Click on Save
   await page.locator(".forceActionButton[title='Save']").click()
   // Verify the first name
   await expect(page.locator(".toastMessage")).toContainText("vinura")
